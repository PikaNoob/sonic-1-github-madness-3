...
@movingdown:
		cmpi.b	#id_AirSlash,obAnim(a0) ; GIO: is Sonic performing the Air Slash?
		beq.s	@success				; GIO: if yes, branch
		cmpi.b  #id_Dropdash,obAnim(a0)	; GIO:is Sonic Drop Dashing?
		beq.s   @success			; if yes, branch
...
React_Enemy:
		...
		cmpi.b	#id_AirSlash,obAnim(a0) ; Is Sonic performing an Air Slash?
...
;		bne.s	@dontdie	; if yes, branch
		...
		cmpi.b	#id_LavaTag,(a2) ; check to see if you were killed by a Lava Tag
		bne.s	@killsoniccont ; if no, branch
		move.b	#id_Burnt,obAnim(a0)
		
	@killsoniccont:
		bset	#7,obGfx(a0)
		move.w	#sfx_Death,d0	; load normal hit sound into D0
		cmpi.b	#id_Spikes,(a2)	; check	if you were killed by spikes
		bne.s	@sound
		move.w	#sfx_HitSpikes,d0 ; load spikes hit sound into D0

	@sound:
		jsr	(PlaySound_Special).l ; play the sound in D0
